...
public class TestComplexTypesHandlingInJava extends SQLRepositoryTestCase {
    ...
    public void verifySchemaDeployed() throws Exception {
        ...
        // in 5.7 restrictions are now loaded in type def !
        SimpleTypeImpl st = (SimpleTypeImpl) ct.getField("fieldType").getType();
        Constraint[] constraints = st.getConstraints();
        ...
        assertTrue(ec.getPossibleValues().contains("string"));
        assertTrue(ec.getPossibleValues().contains("date"));
        assertTrue(ec.getPossibleValues().contains("integer"));
        assertTrue(ec.getPossibleValues().contains("real"));
